...
# 게임을 시작하는 함수(메인)
def runGame():
    germ_image = pygame.image.load('images/germ.png')
    germ_image = pygame.transform.scale(germ_image, (50, 50))      # 세균이미지 크기 조정

    # 충돌 판정용 사각형 제작
    germrect = pygame.image.load('images/germrect.png')
    ...
    while not done:    # 무한 루프 시작
        ...
        if 40 <= total_time - elapsed_time <= 41:
            draw_text('speed up!',
                      pygame.font.Font("Marvel-Regular.ttf", 150), screen, size[0] / 2, size[1] / 2, (0, 0, 0))
