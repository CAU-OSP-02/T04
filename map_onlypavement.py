...
width = 800
height = 600
screen = pygame.display.set_mode((width, height))   # 2. 화면 설정
...
def runMap2(a):
    ...
    while not done:    # 무한 루프 시작
        ...
        # 보도블럭 위만 걷기
            ...
            person.left = width / 3
        elif person.right > width / 3 * 2 and person.bottom > height / 3 * 2:
            if event.key == pygame.K_RIGHT:
                person.right = width / 3 * 2
            if event.key == pygame.K_DOWN:
                person.bottom = height / 3 * 2
        elif person.right > width / 3 * 2 and person.bottom < height / 3:
            if event.key == pygame.K_RIGHT:
                person.right = width / 3 * 2
...
def runMap6(a):
    ...
    while not done:    # 무한 루프 시작
        ...
        if person.left < 0:
            ...
            person.top = height - person.height
        # 보도블럭 위만 걷기
        elif person.left < width/3 and person.bottom > height/3*2:
...
def runMap8():
    ...
    while not done:    # 무한 루프 시작
        ...
        screen.blit(building_WH, (400, 200))
